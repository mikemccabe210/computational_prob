...
def compute_empirical_distribution(values):
    ...
    distribution.update({x:counts[x]/len(values) for x in counts})
    #
    # END OF YOUR CODE
    # -------------------------------------------------------------------------
...
def compute_empirical_mutual_info_nats(var1_values, var2_values):
    ...
    x_dist = compute_empirical_distribution(var1_values)
    y_dist = compute_empirical_distribution(var2_values)
#    print(x_dist)
    xy_dist = compute_empirical_distribution(list(zip(var1_values, var2_values)))
#    print(xy_dist)
    for x,y in xy_dist:
        empirical_mutual_info_nats += xy_dist[(x,y)] * np.log(xy_dist[(x,y)] / (x_dist[x] * y_dist[y]))
...
def chow_liu(observations):
    ...
    for i in range(num_vars):
        for j in range(i+1, num_vars):
            MI_table.append(((i, j), compute_empirical_mutual_info_nats(observations[:, i], observations[:, j])))
    MI_table.sort(key = lambda x: -x[1])
    
    edge_count = 0
    while edge_count < num_vars - 1:
#        print(edge_count)
        for i, ((x, y), cost) in enumerate(MI_table):
#            print(x, y)
...
def compute_empirical_conditional_distribution(var1_values, var2_values):
    ...
    for x2 in conditional_distributions:
        sub_set = var1_values[var2_values == x2]
        counts = Counter(sub_set)
        conditional_distributions[x2].update({x: counts[x]/sub_set.shape[0] for x in counts})
...
def learn_tree_parameters(observations, tree, root_node=0):
    ...
    def process_node(parent, child, node_pot):
        if child not in node_potentials:
            node_potentials[child] = node_pot
            edge_pot = compute_empirical_conditional_distribution(observations[:,child], observations[:,parent])
            edge_potentials[(parent, child)] = edge_pot
            edge_potentials[(child, parent)] = transpose_2d_table(edge_pot)
#            print(edge_pot)
#            print(transpose_2d_table(edge_pot))
            
            for edge in edges[child]:
                process_node(child, edge, node_pot)
    
    node_potentials[root_node] = compute_empirical_distribution(observations[:,root_node])
    node_pot = {x: 1 for x in node_potentials[root_node]}
    processed_nodes.append(root_node)
    for edge in edges[root_node]:
        process_node(root_node, edge, node_pot)
...
def sum_product(nodes, edges, node_potentials, edge_potentials):
    ...
    messages = {}
#    print('edges', edges)
#    print('edge potentials', edge_potentials)
#    print('node potentials', node_potentials)
    ...
    def normalize_dict(dist):
        norm_dict = {}
        Z = sum(dist.values())
        for x in dist:
            norm_dict[x] = dist[x] / Z
        return norm_dict
    # work from leaves to root
    def calc_message(child, root):
        '''
        Calculates a message from a child to a root
        returns marginal dist dict
        '''     
        if (child, root) in messages: 
            return messages[(child, root)]
        else:
            messages[(child, root)] = {}
        # Calculate messages
        if child in leaves:
            for i in node_potentials[root]:       
                partial_message = 0
                for j in edge_potentials[(child, root)]:
                    partial_message += edge_potentials[(child, root)][j][i] * node_potentials[child][j]
                messages[(child, root)][i] = partial_message
            messages[(child, root)] = normalize_dict(messages[(child, root)])
            return messages[(child, root)] 
        else:
            for i in node_potentials[root]:  
                partial_message = 0
                for j in edge_potentials[(child, root)]:
                    sub_partial = edge_potentials[(child, root)][j][i] * node_potentials[child][j]
                    for neighbor in edges[child]:
                        if neighbor != root:
                            sub_partial *= calc_message(neighbor, child)[j]
                    partial_message += sub_partial
                messages[(child, root)][i] = partial_message
            messages[(child, root)] = normalize_dict(messages[(child, root)])
            return messages[(child, root)]
                   
                    
        # calculate this message
    for node in nodes:
        marginals[node] = {}
        for i in node_potentials[node]:
            marginals[node][i] = node_potentials[node][i]
            for child in edges[node]:
                marginals[node][i] *= calc_message(child, node)[i]
        marginals[node] = normalize_dict(marginals[node])
...
def compute_marginals_given_observations(nodes, edges, node_potentials,
    ...
    for obs in observations:
        for outcome in node_potentials[obs]:
            if observations[obs] == outcome:
                node_potentials[obs][outcome] = 1
            else:
                node_potentials[obs][outcome] = 0
    #
    # END OF YOUR CODE
    # -------------------------------------------------------------------------
