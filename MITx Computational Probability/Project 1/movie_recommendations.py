...
def compute_posterior(prior, likelihood, y):
    ...
    observed_likelihood = log_likelihood[y].sum(axis=0)
    numerator = observed_likelihood + log_prior
    
    log_answer = numerator - scipy.misc.logsumexp(numerator)
...
def infer_true_movie_ratings(num_observations=-1):
    ...
    movie_id_list = movieDB.movie_list
   ...
   # MAP_ratings = np.zeros(num_movies)
    for i, m in enumerate(movie_id_list):
        ratings = movieDB.get_ratings(m)[:num_observations]
        posteriors[i] = compute_posterior(prior, likelihood, ratings)
    MAP_ratings = posteriors.argmax(axis = 1)
...
def compute_entropy(distribution):
    ...
    #
    if np.all(np.abs(1 - np.sum(distribution, axis = 0)) > 1e-6):
    ...
    # - use log base 2
    # - enforce 0log0 = 0
    distribution[distribution <= 1e-8] = 0
    log_dist = np.log2(distribution)
    log_dist[log_dist == -np.inf] = 0
    entropy = -(distribution *log_dist).sum(axis = 0)
...
def compute_true_movie_rating_posterior_entropies(num_observations):
    ...
    posterior, _ = infer_true_movie_ratings(num_observations)
    posterior_entropies = compute_entropy(posterior.T)
...
def plot_entropies():
    num_entropy = [compute_true_movie_rating_posterior_entropies(n).mean() for n in range(1,201)]
    plt.figure(1)
    plt.title("Average of Entropies")
    plt.xlabel("Number of Samples")
    plt.ylabel("E (bits)", rotation='horizontal', position=(0, 1.01))
    plt.plot(range(1,201), num_entropy)
    plt.show()
...
def main():
    ...
    distribution = np.array([0.75, 0.25, 0])
    print("My answer:")
    print(compute_entropy(distribution))
    print("Expected answer:")
    print(0.811278124459)
